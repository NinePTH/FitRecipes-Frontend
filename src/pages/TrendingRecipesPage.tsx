...
export function TrendingRecipesPage() {
  ...
  useEffect(() => {
    // Scroll to top when page loads or time range changes
    window.scrollTo(0, 0);

    const fetchRecipes = async () => {
      setLoading(true);
      try {
        const response = await getTrendingRecipes(RECIPES_PER_PAGE, timeRange);
  ...
  const loadMoreRecipes = async () => {
    ...
    try {
      // Note: Backend might need pagination support for trending recipes
      const response = await getTrendingRecipes(RECIPES_PER_PAGE * (currentPage + 1), timeRange);
    ...
    <Link to={`/recipe/${recipe.id}`}>
      <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-200 cursor-pointer">
        <div className="aspect-video relative bg-gray-200">
          ...
          <div className="absolute top-2 left-2 bg-orange-100 text-orange-800 px-2 py-1 rounded-full text-xs font-medium flex items-center space-x-1">
      ...
      <div className="space-y-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No trending recipes yet</h3>
            <p className="text-gray-600">
              Check back later to see what's trending in the community
            </p>
            <Link to="/browse-recipes">
              <Button className="mt-4">Browse All Recipes</Button>
            </Link>
          </div>
        ) : (
          <>
